...
switch ($mode) {
    case 'getMainDevice' :
        ...
        if($_POST['dvcsBldn']) $regist_seq = $_SESSION['user_seq'];
    ...
    case 'getMainDevice_yeom' :
        ...
        foreach($postJson as $key => $val):
            $org_code = $val['org_code'] ?? $_SESSION['user_orgCode'];
            ...
            if($getDeviceList['data'] != null) {
                $getDeviceList['data'][0]['org_name'] = $val['org_name'];
                $resultArray = array_merge($resultArray, $getDeviceList['data']);
            }
        endforeach;

        $deviceListJson = json_encode($resultArray);
        echo $deviceListJson;
        break;
    case 'getMainDevice2' :
        if ($_SESSION["user_auth"] == 4) {    //관리자 A
        ...
        for($i=0; $i<count($exDeviceArr); $i++ ) {
            ...
            $deviceArr[$exDeviceArr[$i]] = $getDeviceList['data'];
        }

        $deviceListJson = json_encode($deviceArr);
    ...
    case 'getGWDevice' :    //Gateway 리스트
        ...
        foreach ($apiResult[data] as $key => $gwList) {
            ...
            <tr>
                ...
                <td rowspan="2">
                    <label class="switch">
                        <input type="checkbox" <?= $gatewayUseYN ?> class="useChange"
                               value="<?= $gwList["GatewayUseYN"] ?>">
                        <span></span>
                    </label>
                </td>

                <td>
                    <input class="form-control inputChange" placeholder="Comment" autocomplete="off"
                           name="<?= $gwName ?>" value="<?= $gwList[GatewayName] ?>" disabled>
                </td>
                ...
                for ($i = 0; $i < 3; $i++) {
                    ...
                    ?>
                    <td>
                        <select class="custom-select custom-select-sm outChange" name="<?= $gwName ?>"
                                data-outch="<?= $i + 1 ?>">
                ...
                <td>
                    <label class="switch d-block m-0">
                        <input type="checkbox" <?= ($gwList[tempYN] == '1') ? "checked" : ""; ?> class="tempModal"
                               name="<?= $gwName ?>" value="<?= $gwList[tempYN] ?>">
                        <span></span>
                    </label>
                </td>
                <!-- 습도 -->
                <td>
                    <label class="switch d-block m-0">
                        <input type="checkbox" <?= ($gwList[huYN] == '1') ? "checked" : ""; ?> class="huModal"
                               name="<?= $gwName ?>" value="<?= $gwList[huYN] ?>">
                        <span></span>
                    </label>
                </td>
                <!-- Gas -->
                <td>
                    <label class="switch d-block m-0">
                        <input type="checkbox" <?= ($gwList[gasYN] == '1') ? "checked" : ""; ?> class="gasModal"
                               name="<?= $gwName ?>" value="<?= $gwList[gasYN] ?>">
                        <span></span>
                    </label>
                </td>
                <td rowspan="2">
            ...
            <tr>
                <td>
                    <input class="form-control cmt" placeholder="Comment" autocomplete="off" name="<?= $cmtName ?>"
                           value="<?= $gwList[GatewayCmt] ?>">
                </td>
                <?
                for ($i = 0; $i < 6; $i++) {
                    $inchCmtNum = $i + 1;
    ...
    case 'updateGW' :        //Gateway 수정
        ...
        $postData['org_name'] = $_POST['org_name'] ?? '';
    ...
    case 'updateOutch' :        //출력 채널 On/Off 바꾸기
        $postData = Array(
            ...
            "outResult1" => $_POST["outResult1"],
            "outResult2" => $_POST["outResult2"],
            "outResult3" => $_POST["outResult3"],
            "id" => $_SESSION["user_id"],
            "name"=>$_SESSION["user_name"],
    ...
    case 'allDeleteGW' : //gateway 일괄 삭제
        ...
        break;
    case 'registGwByCH' : //복구
        $postData = Array(
            "SerialNo" => ($_POST["SerialNo"] == "")?"":$_POST["SerialNo"],
            "GatewayKey" => ($_POST["GatewayKey"] == "")?"":$_POST["GatewayKey"],
            "chType"=> ($_POST["chType"] == "")?"":$_POST["chType"],
            "org_code"=>($_POST["org_code"] == "")?"":$_POST["org_code"],
            "bldgName"=>($_POST["bIdgName"]  == "")?"":$_POST["bIdgName"],
            "gatewayComment"=>($_POST["gwCmt"] == "")?"":$_POST["gwCmt"],
            "comment"=> ($_POST["comment"] == "")?"":$_POST["comment"],
            "name"=>($_SESSION["user_name"] == "")?"":$_SESSION["user_name"],
            "id"=>($_SESSION["user_id"] == "")?"":$_SESSION["user_id"]
        );

        $apiResult = registGwByCH($postData);
